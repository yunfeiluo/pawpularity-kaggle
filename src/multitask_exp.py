...
def get_pretrained_out(feature_extractor, train_dataloader, val_dataloader, batch_size, device):
...
def train_model(regressor, train_loader, val_loader, device, epochs=30, lr=1e-4):
...
if __name__ == '__main__':
    ...
    train_model(regressor, train_pre_loader, val_pre_loader, device, epochs=10, lr=1e-4)

    print('Construct Integrated Model...')
    model = IntegratedModel(device, regressor=regressor).to(device)
    print('Num params:', sum(p.numel() for p in model.parameters() if p.requires_grad))
    train_model(model, train_dataloader, val_dataloader, device, epochs=10, lr=1e-4)
